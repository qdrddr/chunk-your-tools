#!/usr/bin/env bash
# Run license audits for all ecosystems in the chunk-your-tools repo.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
	...
	--install-tools)
		...
		;;
	--skip)
		[[ $# -ge 2 ]] || legal_die "--skip requires rust|python|npm|go|c"
		case "$2" in
		rust) SKIP_RUST=1 ;;
		...
		go) SKIP_GO=1 ;;
		c) SKIP_C=1 ;;
		*) legal_die "unknown --skip target: $2 (expected rust|python|npm|go|c)" ;;
		esac
		shift 2
		;;
	--skip=*)
...
Options:
  ...
  --skip TARGET      Skip rust, python, npm, go, or c (repeatable)

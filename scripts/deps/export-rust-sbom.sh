#!/usr/bin/env bash
# Export CycloneDX SBOM for chunk-your-tools and Snyk SBOM test report.
#
# Generated from Cargo.lock (do not edit by hand):
#   chunk-your-tools.cdx.json   — CycloneDX SBOM (cargo cyclonedx)
#   chunk-your-tools.snyk.json  — Snyk SBOM test result
#
# Pre-commit auto-fix hooks exclude these paths (see .pre-commit-config.yaml);
# typos exclusions are in typos.toml; manual prettier uses .prettierignore.
#
# Usage:
#   ./scripts/deps/export-rust-sbom.sh
#   ./scripts/deps/export-rust-sbom.sh --check
#   ./scripts/deps/export-rust-sbom.sh --output-dir DIR
...
CDX_FILE="${REPO_ROOT}/chunk-your-tools.cdx.json"
SNYK_FILE="${REPO_ROOT}/chunk-your-tools.snyk.json"
DO_CHECK=0
OUTPUT_DIR=""
...
while [[ $# -gt 0 ]]; do
	...
	-h | --help)
		cat <<'EOF'
Usage: export-rust-sbom.sh [--check] [--output-dir DIR]

Writes chunk-your-tools.cdx.json and chunk-your-tools.snyk.json.
...
sanitize_snyk() {
	...
	jq --arg root "${repo_root}" '
		.displayTargetFile = "chunk-your-tools.cdx.json"
		| .targetFile = "chunk-your-tools.cdx.json"
		| .path = "."
		| walk(
			if type == "string" then
				gsub("^" + $root + "/"; "")
...
export_rust_sbom() {
	local cdx_out="$1"
	local snyk_out="$2"
	local cdx_tmp snyk_tmp snyk_status cdx_raw cdx_backup="" snyk_backup=""
	...
	(
		cd "${REPO_ROOT}"
		run_cmd cargo cyclonedx --manifest-path Cargo.toml --format json
	)
	...
	assert_no_absolute_paths "${cdx_tmp}" "chunk-your-tools.cdx.json"
	...
	(
		...
		run_cmd snyk sbom test \
			--file="chunk-your-tools.cdx.json" \
			--include-ignores \
	...
	assert_no_absolute_paths "${snyk_tmp}.sorted" "chunk-your-tools.snyk.json"
...
if [[ "${DO_CHECK}" -eq 1 ]]; then
	...
	export_rust_sbom "${tmp_dir}/chunk-your-tools.cdx.json" "${tmp_dir}/chunk-your-tools.snyk.json"

	status=0
	for name in chunk-your-tools.cdx.json chunk-your-tools.snyk.json; do
		...
		if ! files_match "${committed}" "${generated}"; then
			echo "error: ${name} is out of sync (run: ./scripts/deps/export-rust-sbom.sh)" >&2
			status=1
		fi
	done
	exit "${status}"
...
if [[ -n "${OUTPUT_DIR}" ]]; then
	mkdir -p "${OUTPUT_DIR}"
	export_rust_sbom "${OUTPUT_DIR}/chunk-your-tools.cdx.json" "${OUTPUT_DIR}/chunk-your-tools.snyk.json"
	exit 0
fi
